...
checks:
  ...
  - type: "test"
    ...
    var: "CURSOR_API_KEY"
    optional: true
    test: "sh -c 'if [ -z \"$CURSOR_API_KEY\" ]; then echo \"SKIPPED (no CURSOR_API_KEY)\"; exit 0; fi; base=\"${CURSOR_API_BASE_URL:-https://api.cursor.sh}\"; code=\"$(curl -s -o /dev/null -w \"%{http_code}\" \"$base/v1/models\" -H \"Authorization: Bearer $CURSOR_API_KEY\" || echo 000)\"; case \"$code\" in 200|429) echo \"OK (HTTP $code)\"; exit 0 ;; 401|403) echo \"INVALID (HTTP $code)\"; exit 1 ;; 502|503|504|000) echo \"UNAVAILABLE (HTTP $code)\"; exit 0 ;; *) echo \"FAILED (HTTP $code)\"; exit 1 ;; esac'"
---
# Cursor Module
