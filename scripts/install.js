...
const argv = yargs(hideBin(process.argv))
  ...
  .option('test-checks', {
    type: 'string',
    description: 'Comma-separated list of checks to test (without installing)'
  })
  .option('checks', {
    type: 'string',
...
const WORKSPACE_PATH = argv.workspacePath;
const INSTALL_MODULES = argv.modules;
const AI_AGENT = argv.aiAgent;
const REPO_TYPE = argv.repoType;
const SKIP_REPO_INIT = argv.skipRepoInit;
const CONFIG_FILE_PATH = argv.config;
const AUTO_YES = argv.yes || argv.nonInteractive || argv.unattended;
const CHECK_TOKENS_ONLY = argv.checkTokensOnly;
const STATUS_ONLY = argv.status;
const TEST_CHECKS = argv.testChecks ? argv.testChecks.split(',').map(c => c.trim()).filter(c => c.length > 0) : null;
const CHECKS = argv.checks ? argv.checks.split(',').map(c => c.trim()).filter(c => c.length > 0) : null;
