 ...
 */
function parseModuleFrontmatter(modulePath) {
 ...
 */
function resolveDependencies(moduleNames, allModules) {
  ...
  return Array.from(resolved).map(name => moduleMap.get(name)).filter(Boolean);
}

/**
 * Resolve modules from workspace config
 */
function resolveModules(workspaceConfig, allModules) {
 ...
 */
function mergeModuleSettings(module, workspaceModuleSettings) {
