...
// Load .env file from workspace root (best-effort)
...
test.describe('issue-tracker-github: github-provider', () => {
  ...
  test('downloadResources creates correct directory structure', async () => {
    test.skip(!hasGitHubToken(), 'GITHUB_TOKEN not set');
    const workspaceRootLocal = findWorkspaceRoot(process.cwd()) || process.cwd();
    ...
    const issueDir = getIssueCacheDir(workspaceRootLocal, TEST_ISSUE_ID);
    const resourcesDir = getIssueResourcesDir(workspaceRootLocal, TEST_ISSUE_ID);
    const resourcesJsonPath = getResourcesJsonPath(workspaceRootLocal, TEST_ISSUE_ID);
  ...
  test('downloadResources creates resources.json with correct metadata', async () => {
    ...
    const resourcesJson = readResourcesJson(workspaceRootLocal, TEST_ISSUE_ID);
